...
import { Annonce, getAllAnnonces, setCurrentPage } from '@/store/slices/annonceSlice';
...
export default function Home() {
  ...
  // Filtering logic
  const filteredAnnonces = annonces.filter((annonce: Annonce) => {
  if (filters.marque && annonce.brand?.toLowerCase() !== filters.marque.toLowerCase()) return false;
  if (filters.modele && annonce.model?.toLowerCase() !== filters.modele.toLowerCase()) return false;
  if (filters.annee && String(annonce.year) !== filters.annee) return false;
  if (filters.prixMin && annonce.price < Number(filters.prixMin)) return false;
  if (filters.prixMax && annonce.price > Number(filters.prixMax)) return false;
  if (filters.kilometrageMin && annonce.mileage < Number(filters.kilometrageMin)) return false;
  if (filters.kilometrageMax && annonce.mileage > Number(filters.kilometrageMax)) return false;
  if (filters.localisation && annonce.location?.toLowerCase() !== filters.localisation.toLowerCase()) return false;
  if (filters.carburant && annonce.fuelType?.toLowerCase() !== filters.carburant.toLowerCase()) return false;
  if (filters.transmission && annonce.transmission?.toLowerCase() !== filters.transmission.toLowerCase()) return false;
  return true;
});
  ...
  useEffect(() => {
  dispatch(setCurrentPage(1)); // reset à la première page quand les filtres changent
}, [filters, dispatch]);
